...
pub fn inputlag_mouse_keyboard() -> Result<()> {
    ...
    let inputlag_mouse = hkcu.open_subkey_with_flags("Control Panel\\Accessibility\\MouseKeys", KEY_SET_VALUE)?;
    inputlag_mouse.set_value("Flags", &0u32)?;

    let inputlag_keyboard = hkcu.open_subkey_with_flags("Control Panel\\Accessibility\\Keyboard Response", KEY_SET_VALUE)?;
    inputlag_keyboard.set_value("Flags", &0u32)?;

    let sticky_keys = hkcu.open_subkey_with_flags("Control Panel\\Accessibility\\StickyKeys", KEY_SET_VALUE)?;
    ...
    let smooth_mouse_x_curve = RegValue{ vtype: REG_BINARY, bytes: bytes};
    ...
    let smooth_mouse_y_curve = RegValue{ vtype: REG_BINARY, bytes: bytes};
    ...
    let default_user_key = hkuser.open_subkey_with_flags(".DEFAULT\\Control Panel\\Mouse", KEY_SET_VALUE)?;
